...
Реальные документы от заказчика придут позже и лягут в `fixtures/real-contracts/`
...
def contract_07_dates() -> tuple[DocxDocument, list[dict[str, str]]]:
    ...
    doc.add_paragraph(
        "Продавец: Сидорова Анна Петровна, 15 февраля 1982 года рождения, паспорт РФ."
    )
    ...
    doc.add_paragraph("Договор от 10 марта 2025 г., свидетельство о рождении выдано ранее.")
    # Отрицательные примеры — не должны попасть ни в один тип.
    doc.add_paragraph("Незаполненный шаблон подписи: «____» __________ 202__ года — не дата.")
