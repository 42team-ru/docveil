...
и на 13 строках терминала помещается силуэт, который читается как щит, а не
как набор палочек.
...
def _colour(x: int, y: int) -> tuple[int, int, int] | None:
    ...
    for row, right in ((0.24, 0.66), (0.355, 0.78), (0.47, 0.64)):
        top = _H * row
        if top <= y < top + 2 and _W * 0.20 <= x <= _W * right:
            return BAR
