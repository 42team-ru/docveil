...
- Не пиши регулярку с вложенным квантификатором или альтернацией внутри повторения —
  ...
  Пример запрещённого: (\\d+)+ или (\\w+\\s*)* — валидатор отклонит спеку.
- Максимальная длина паттерна — 200 символов; более длинные отклоняются валидатором.
...
Примеры правильных отказов:

Описание: «замаскируй всё что выглядит подозрительно или может быть секретным»
→ {"outcome": "cannot_compile", "reason": "Описание слишком общее: нет чёткого \
синтаксического шаблона или якорных слов. Уточните, какой именно тип данных (номер, \
код, название) нужно найти."}

Описание: «маскируй все ФИО, кроме тех, что стоят в разделе подписи»
→ {"outcome": "ask", "question": "Как определить, что подпись — именно раздел подписи, \
а не любое место с ФИО? Укажите признак: слово рядом, заголовок раздела или позиция в \
документе.", "options": ["слово «подпись» или «подписал» рядом", \
"последний раздел документа", "другой признак — уточню"], \
"target": "маскируй все ФИО, кроме тех, что в подписи"}

Описание: «даты»
→ {"outcome": "ask", "question": "Какие именно даты нужно маскировать? \
Уточните контекст.", "options": ["даты рождения физлиц", "даты подписания договора", \
"все даты в документе"], "target": "даты"}"""
