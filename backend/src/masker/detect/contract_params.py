...
# 49, 84, 125, 125 и 204 символа. Предел 160 сохранял короткие факты и
# отсекал длинные захваты.
#
# 14.09.2026: предел поднят до 320. Отсечённый 313-символьный спан —
# не «абзацный захват», а размеченное условие оплаты целиком
# (`contract_pdf_02_school`: «оплата производится … и счёта Исполнителя на
# оплату,»). При 160 recall payment_terms на корпусе был 0.750 при пороге
# 0.85 — порог не брался по построению. Замер на том же документе: при 320
# находится ровно один дополнительный спан, ложных не добавилось.
_MAX_PAYMENT_TERMS_LENGTH = 320
...
_DELIVERY_PATTERNS = [
    ...
    # 12.09.2026: срок продления приёмки «не более чем на 5 (Пять)
    # рабочих дней» так же раскрывает коммерческий срок, хотя не начинается
    # с «в течение» или «не позднее».
    re.compile(
        r"не\s+более\s+чем\s+на\s+\d+\s*(?:\([^)]+\)\s*)?"
        r"(?:рабочих|календарных)?\s*(?:дн[её]й|дня|суток)",
        re.IGNORECASE,
    ),
