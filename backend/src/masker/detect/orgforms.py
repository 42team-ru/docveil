...
# Допускает строчные союзы-коннекторы: «ГО и ЧС», «СНиП» (буква «и» внутри слова тоже).
_RE_REGULATORY_CODE = re.compile(
    r"^[А-ЯЁA-Z]{2,4}(\s+(и|или)\s+[А-ЯЁA-Z]{2,4}|\s+[А-ЯЁA-Z]{1,4}){1,3}$"
)
...
def is_public_body(text: str) -> bool:
    """Вернуть True если хотя бы один токен совпадает со стемом публичного органа.

    ANY-matching: "Администрации города" попадает по "администраци", даже если
    "города" ни с чем не совпадает. До этого ALL-matching пропускал такие фразы.
    """
