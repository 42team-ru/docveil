...
class Segment:
    ...
    ``origin`` различает происхождение сегмента для рендера: ``"text"`` —
    настоящий текстовый слой (docx-параграф, ячейка xlsx, текстовая
    страница PDF) со всеми существующими сегодня контрактами якоря;
    ``"ocr"`` — строка, распознанная OCR со скан-страницы PDF, у которой
    якорь несёт bbox в pt страницы, а не символьный диапазон; ``"user"`` —
    искусственный сегмент bbox-правки оператора (план feat/highlight-
    coords-edits, К2): якорь тоже несёт bbox, но текст не распознан OCR, а
    напечатан человеком — `masker.report.coverage` намеренно не считает его
    в OCR-покрытии. Детекция, политика и план значения не читают — им
    прозрачно, откуда пришёл текст. Значение по умолчанию ``"text"`` не
    меняет ни один существующий вызов ``Segment(...)`` (T2.3, шаг O1).
    """
...
class MaskGroup:
    ...
    #: Человекочитаемая полная метка — роль обычным регистром вперёд типа,
    #: слова через пробел (план М4): ``"Заказчик Представитель"``, а не
    #: капс-с-дефисами ``marker`` (``mask/labels.py::compose_canonical_label``).
    #: Раздельные имена нужны, потому что ``marker`` — уже существующий
    #: контракт eval.py/validate/report, трогать который нельзя, а
    #: ``canonical_label`` — то, что реально печатается в PDF.
    canonical_label: str = ""
...
class MarkerRenderResult:
    ...
    другое значение — какая ступень лестницы сработала («role_only» |
    «compact» | «type_only» | «blank»). Отчёт превращает непустой
    ``fallback_reason`` в строку легенды вида
    ``[Ф1] = [Поставщик Представитель], стр. 3``.
    """
