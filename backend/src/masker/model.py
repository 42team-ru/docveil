...
class Segment:
    ...
    якорь несёт bbox в pt страницы, а не символьный диапазон. Детекция,
    политика и план значения не читают — им прозрачно, откуда пришёл
    текст. Значение по умолчанию ``"text"`` не меняет ни один существующий
    вызов ``Segment(...)`` (T2.3, шаг O1).
    """
