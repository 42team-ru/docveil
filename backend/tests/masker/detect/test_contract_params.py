...
def test_payment_terms_rejects_paragraph_longer_than_fact_limit() -> None:
    """Абзацный захват не является условием оплаты.

    14.09.2026: предел поднят со 160 до 320 символов — 313-символьное
    условие из `contract_pdf_02_school` размечено в корпусе целиком, и при
    160 recall payment_terms не брал порог по построению. Проверка та же:
    захват ЗАМЕТНО длиннее предела по-прежнему отбрасывается.
    """
    text = "Порядок оплаты. Оплата производится " + ("Заказчиком по факту оказания услуг " * 16)
    assert len(text[text.index("Оплата") :]) > 320
