...
def test_multi_inn_profiles_do_not_grow_beyond_known_debt() -> None:
    ...
    14.09.2026: один из десяти случаев (`ipklh-2022-01-11.pdf`) оказался не
    реквизитом казначейства, а ложным ИНН — `normalize_layout.py::
    _split_concatenated_kpp_inn` резал слипшийся 19-значный хвост на КПП+ИНН
    по всему сегменту, а не рядом с меткой «ИНН»/«КПП», поэтому 20-значный
    «Единый казначейский счёт» иногда терял мнимый ИНН по случайному
    совпадению контрольной суммы (~1/10 шанс на произвольных 10 цифрах).
    Правило сузили до метки в пределах 40 символов перед хвостом — этот
    случай исчез, порог снижен на 1."""
    KNOWN_DEBT = 9
