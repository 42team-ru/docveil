...
def is_valid_bik(value: str) -> bool:
    ...
    №579-П): `04` — сам ЦБ РФ, `01` — с 2022 года подразделения
    Федерального казначейства, обслуживающие бюджетные счета. Раньше
    правило принимало только `04` и отбрасывало валидные казначейские
    БИК вроде `016577551` — не потому что у них «неправильная контрольная
    сумма» (её и не считали), а потому что префикс не был внесён в код.
    """
    d = _digits(value)
    return d is not None and len(d) == 9 and d[0] == 0 and d[1] in (1, 4)
